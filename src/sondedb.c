...
extern void senddata_db(sondeaprs_type type, double lat, double long0, double alt,
                ...
                unsigned long txtime, unsigned char burstKill, const char* sd_raw,
        unsigned long sd_raw_len, char* hwType, char* hwRev, char* hwSN, 
        char* presSN, char* fwVersion, double vBatt, double tempInt, 
        char* flightState, unsigned char heating, unsigned char power, char* error, unsigned long ip)
{
    ...
    make_number_or_null_d(sd_press, MEAS_LEN, hp, (double)X2C_max_real);
    make_number_or_null_d(sd_hyg, MEAS_LEN, hyg, (double)X2C_max_real);
    make_number_or_null_d(sd_temp, MEAS_LEN, temp, (double)X2C_max_real);
    make_number_or_null_d(sd_bat_voltage, MEAS_LEN, vBatt, 0.0);
    ...
    struct tm tm_sat_time = *gmtime(&sattime);
    strftime(sd_sat_time, sizeof(sd_sat_time), "'%T'", &tm_sat_time);
    make_number_or_null_t(sd_uptime, MEAS_LEN, uptime, 86401);
    struct tm tmkt = *gmtime(&txtime);
    strftime(sd_kill_timer, sizeof(sd_kill_timer), "'%T'", &tmkt);
